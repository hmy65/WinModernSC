...
# ============================================================ 改造
def limit_source_vf(font, shape):
...
def convert(font, real, shape):
    ...
    rep = patch_glyphs.patch(font, real, REAL, real["OS/2"].usWeightClass,
...
def main():
    ...
    try:
        ...
        if src.kind == util.STATIC:
            ...
            font = TTFont(src.vf)
            font = limit_source_vf(font, shape)
            how = "源自带的可变字体"
        log("")
        convert(font, real, shape)
    finally:
        real.close()
    ...
    # 生成完当场自检，不通过就非零退出
    verify_fonts.report(*verify_fonts.verify_vf(out))
